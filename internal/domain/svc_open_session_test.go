...
func TestOpenSession_Open(t *testing.T) {
	makeOpener := func(idGenErr, tokenGenErr, hashErr error) IOpenSession {
	...
	t.Run("happy_path_new_session", func(t *testing.T) {
		svc := makeOpener(nil, nil, nil)
		userID := validUserID()

		rawTok, sess, revokedSess, err := svc.Open(nil, nil, userID, differentDeviceIdentity(), testNow)
	...
	t.Run("existing_fingerprint_session_updated", func(t *testing.T) {
		...
		_, sess, revokedSess, err := svc.Open(existingSession, nil, userID, validDeviceIdentity(), testNow)
		require.NoError(t, err)
	...
	t.Run("session_limit_exceeded_revokes_oldest", func(t *testing.T) {
		...
		_, sess, revokedSess, err := svc.Open(nil, []*Session{oldestSession}, validUserID(), differentDeviceIdentity(), testNow)
		require.NoError(t, err)
	...
	t.Run("id_gen_fails", func(t *testing.T) {
		svc := makeOpener(errors.New("id gen failed"), nil, nil)

		_, _, _, err := svc.Open(nil, nil, validUserID(), differentDeviceIdentity(), testNow)
		assert.Error(t, err)
	})

	t.Run("token_gen_fails", func(t *testing.T) {
		svc := makeOpener(nil, errors.New("token gen failed"), nil)

		_, _, _, err := svc.Open(nil, nil, validUserID(), validDeviceIdentity(), testNow)
		assert.Error(t, err)
	})

	t.Run("hash_fails", func(t *testing.T) {
		svc := makeOpener(nil, nil, errors.New("hash failed"))
