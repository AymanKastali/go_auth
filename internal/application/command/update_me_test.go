...
func TestUpdateMeHandler(t *testing.T) {
	t.Run("happy_path", func(t *testing.T) {
		user := testActiveUser()
		h := NewUpdateMeHandler(
			&stubAppUserRepository{findByIDResult: user, findByEmailResult: nil},
			&stubClock{now: appTestNow},
			&mockEventDispatcher{},
		)

		err := h.Handle(authenticatedCtx("user-001", "sess-001"), UpdateMeCommand{Email: "new@example.com"})
	...
	t.Run("unauthenticated", func(t *testing.T) {
		...
		err := h.Handle(unauthenticatedCtx(), UpdateMeCommand{Email: "a@b.com"})
	...
	t.Run("invalid_email", func(t *testing.T) {
		...
		err := h.Handle(authenticatedCtx("user-001", "sess-001"), UpdateMeCommand{Email: "bad"})
	...
	t.Run("user_not_found", func(t *testing.T) {
		...
		err := h.Handle(authenticatedCtx("user-001", "sess-001"), UpdateMeCommand{Email: "a@b.com"})
	...
	t.Run("email_taken", func(t *testing.T) {
		...
		err := h.Handle(authenticatedCtx("user-001", "sess-001"), UpdateMeCommand{Email: "taken@example.com"})
	...
	t.Run("same_email_idempotent", func(t *testing.T) {
		...
		err := h.Handle(authenticatedCtx("user-001", "sess-001"), UpdateMeCommand{Email: "test@example.com"})
		require.NoError(t, err)
	})
